...
function solve(caseNumber, L, tokens) {
  let primes = [];
  primes[1] = gcd(tokens[0], tokens[1]);
  primes[0] = tokens[0] / primes[1];
  for (let i = 2; i < L; i++){
      primes[i] = tokens[i-1] / primes[i-1];
  }
  ...
  for (let i = 0; i < primes.length; i++) {
      text += lettersHash[primes[i]];
  }
  console.log(`Case #${caseNumber}: ${text}`);
  // return;
